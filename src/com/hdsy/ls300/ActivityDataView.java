...
public class ActivityDataView extends Activity implements ViewFactory,
		...
		gestureScanner
				.setOnDoubleTapListener(new GestureDetector.OnDoubleTapListener() {
					public boolean onDoubleTap(MotionEvent e) {
						// 双击时产生一次
						// 启动大图界面
						Intent mainIntent = new Intent(ActivityDataView.this,
								ActivityDataImageView.class);
						ActivityDataView.this.startActivity(mainIntent);
						return false;
					}
	...
	public View makeView() {
		...
		if (Internal.data_mgr.Count() > 0) {
			...
			tv.setText(Internal.data_mgr.GetSelect().toString());
		} else {
	...
	public boolean onTouch(View v, MotionEvent event) {
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if (touchUpX - touchDownX > 100) {
				...
				tv.setText(Internal.data_mgr.GetSelect().toString());
			} else if (touchDownX - touchUpX > 100) {//后一张
				// 取得当前要看的图片的index
